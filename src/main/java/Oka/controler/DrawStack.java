...
import static Oka.model.Enums.Color.values;
...
public class DrawStack
{
    ...
    public static Goal drawGoal (goalType goalType)
    {
        ...
        {
            case BambooGoal:
                return new BambooGoal(2, random.nextInt(3) + 2, PINK);

            case PlotGoal:
                break;

            case GardenGoal:
                return new GardenerGoal(2, random.nextInt(3) + 1, PINK);
    ...
    private static Plot setPlotState (int indexColor, int randInt)
    {
        ...
        Enums.Color color = values()[indexColor];
    ...
    public static void giveBackPlot (ArrayList<Plot> listP)
    {
        ...
        for (int i = 0; i < listP.size(); i++)
        {
            ...
            while (colorPlot.equals(values()[index]))
            {
                index++;
            }
